...
func TestGetPaymentPage(t *testing.T) {
	...
	assert.Contains(t, string(body), "Payment")
	assert.Contains(t, string(body), "Amount (EGP)")
}
...
func TestPaymentSuccess(t *testing.T) {
	...
	// Simulate a successful callback from PayMob (success=true)
	req := httptest.NewRequest("GET", "/success?order_id=test-order-123&success=true", nil)
	resp, err := app.Test(req, -1)
	require.NoError(t, err)
	defer resp.Body.Close()
...
func TestPaymentFailure(t *testing.T) {
	...
	// Simulate a failure callback from PayMob (error_occured=true)
	req := httptest.NewRequest("GET", "/failure?order_id=test-order-456&error_occured=true", nil)
	resp, err := app.Test(req, -1)
	require.NoError(t, err)
	defer resp.Body.Close()

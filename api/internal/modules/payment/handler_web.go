...
// In web mode, responses include HTML rendering via templates.
type Handler struct {
	service  *Service
	repo     *Repository
...
// NewHandler creates a new payment handler with template rendering.
...
func (h *Handler) PaymentSuccess(c *fiber.Ctx) error {
	...
	if hasError && payment.Status == domain.PaymentStatusPending {
		...
		// Mark as success when no error and still pending (demo mode behavior)

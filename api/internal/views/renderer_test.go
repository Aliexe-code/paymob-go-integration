...
func TestRenderPaymentPage(t *testing.T) {
	...
	assert.Contains(t, html, "Payment")
	assert.Contains(t, html, "hx-post=\"/api/payments\"")
	assert.Contains(t, html, "Amount (EGP)")
}
...
func TestFormatAmountFunc(t *testing.T) {
	assert.Equal(t, "0", utils.FormatAmount(0))
	assert.Equal(t, "1", utils.FormatAmount(1))
	assert.Equal(t, "100", utils.FormatAmount(100))
	assert.Equal(t, "1,000", utils.FormatAmount(1000))
	assert.Equal(t, "10,000", utils.FormatAmount(10000))
	assert.Equal(t, "1,000,000", utils.FormatAmount(1000000))
}

func TestStatusClassFunc(t *testing.T) {
	assert.Equal(t, "bg-green-500/20 text-green-400", utils.StatusClass("success"))
	assert.Equal(t, "bg-red-500/20 text-red-400", utils.StatusClass("failed"))
	assert.Equal(t, "bg-yellow-500/20 text-yellow-400", utils.StatusClass("pending"))
	assert.Equal(t, "bg-yellow-500/20 text-yellow-400", utils.StatusClass("unknown"))
}

func TestStatusTextFunc(t *testing.T) {
	assert.Equal(t, "Success", utils.StatusText("success"))
	assert.Equal(t, "Failed", utils.StatusText("failed"))
	assert.Equal(t, "Pending", utils.StatusText("pending"))
	assert.Equal(t, "Cancelled", utils.StatusText("cancelled"))
	assert.Equal(t, "Pending", utils.StatusText("unknown"))
}
